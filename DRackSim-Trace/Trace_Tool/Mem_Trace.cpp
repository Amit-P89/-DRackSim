...
// Number of instruction to trace
KNOB<UINT64> KnobNumInsTrace(KNOB_MODE_WRITEONCE, "pintool", "M", "10000000", "Number of Instructions to Trace");

int Procid;
int Nodeid;
uint64_t INST_to_SKIP=0;
bool only_multi_thread=false;
long int Max_ins_trace = 10000000;
...
static VOID InsRef(UINT32 threadid, ADDRINT addr, UINT32 category, UINT32 opcode)
{
    ...
    if(Inst<INST_to_SKIP)
    {
        return;
    }

    if(Inst>(Max_ins_trace+INST_to_SKIP))
    {
        PIN_ExitApplication(0);
    }
...
static VOID Instruction(INS ins, VOID *v)
{
    ...
    INS_InsertCall(
        ins, IPOINT_BEFORE, (AFUNPTR)InsRef, IARG_THREAD_ID,IARG_INST_PTR,
        IARG_UINT32,INS_Category(ins),IARG_UINT32,INS_Opcode(ins), IARG_END);
...
{     
    ...
    ins_stat<<"\nTotal instructions traced in Node-"<<Nodeid<<" App-"<<Procid<<" : "<<Max_ins_trace<<endl;
    ins_stat<<"\nTotal memory accesses(to cache) by CPU, traced in Node-"<<Nodeid<<" : "<<*win;
    ins_stat<<"\nNum SSE instructions:"<<num_SSE;
    ins_stat<<"\nNum x87_ALU instructions:"<<x87_alu;
    ins_stat<<"\nNum x87_MUL instructions:"<<x87_mul;
    ins_stat<<"\nNum x87_DIV instructions:"<<x87_div;
    ins_stat<<"\nNum int_ALU instructions:"<<int_alu;
    ins_stat<<"\nNum int_MUL instructions:"<<int_mul;
    ins_stat<<"\nNum int_DIV instructions:"<<int_div;
    ins_stat<<"\nNum NOP instructions:"<<nop;
    ins_stat<<endl<<endl;
    PIN_MutexUnlock(Mutex1);
...
extern int main(int argc, char *argv[])
{
    ...
    Max_ins_trace=KnobNumInsTrace.Value();
