...
#define TLB_MISS_PENALTY 60
#define page_table_miss_penalty 10 //can be changed
#define L1_PENALTY 4
#define L2_PENALTY 12
#define L3_PENALTY 25
...
//Get num_insts from the script
KNOB<UINT32> KnobMaxMemRefs(KNOB_MODE_WRITEONCE, "pintool", "I", "10000000", "Max mem Refs");

int Procid;
int Nodeid;
uint64_t max_mem_refs;
...
static std::size_t PenaltyCalc(BOOL, BOOL, BOOL, BOOL, BOOL);
...
unordered_map<uint64_t, int> page_table[CORE_NUM];

UINT64 total_access_count=0;
//function called on each instruction fetch and memory reference
...
static VOID InsRef_No_INS(UINT32 threadid, ADDRINT addr, UINT32 size, CACHE_BASE::ACCESS_TYPE accessType, CHAR r, UINT32 procid)
{
 ...
 /*   const UINT32 size = 1; // assuming access does not cross cache lines
    ...
    BOOL page_table_hit=true;

    if(!itlbHit)
    {
        uint64_t page_addr = addr >> 12;
        if(page_table[threadid % CORE_NUM].find(page_addr) == page_table[threadid % CORE_NUM].end())
        {
            page_table_hit = false;
            page_table[threadid % CORE_NUM][page_addr] = 1;
        }
    }
    ...
    UINT32 penalty= PenaltyCalc(itlbHit, il1Hit, ul2Hit, ul3Hit, page_table_hit);
...
static VOID MemRefMulti_No_INS(UINT32 threadid, ADDRINT addr, UINT32 size, CACHE_BASE::ACCESS_TYPE accessType, CHAR r, UINT32 procid)
{
    ...
    UINT32 penalty=PenaltyCalc(dtlbHit, dl1Hit, ul2Hit,ul3Hit, page_table_hit);
...
static std::size_t PenaltyCalc(BOOL bool_tlbHit , BOOL bool_l1Hit, BOOL bool_l2Hit, BOOL bool_l3Hit, BOOL page_table_hit)
{
    std::size_t int_tlbHit = (bool_tlbHit) ? 1 : 0;
    std::size_t int_tlbMiss = (!bool_tlbHit) ? 1 : 0;
    std::size_t int_l1Hit = (bool_l1Hit) ? 1 : 0;
    std::size_t int_l2Hit = (bool_l2Hit) ? 1 : 0;
    std::size_t int_l3Hit = (bool_l3Hit) ? 1 : 0;
    std::size_t pageTable_Miss = (!page_table_hit) ? 1 : 0;

    uint64_t penalty = max(max((int_tlbHit * TLB_PENALTY), (int_tlbMiss * TLB_MISS_PENALTY)) , ((int_l1Hit * L1_PENALTY) + (int_l2Hit * L2_PENALTY) + (int_l3Hit * L3_PENALTY)));
    penalty = max(penalty, (penalty + (pageTable_Miss*page_table_miss_penalty)));
...
extern int main(int argc, char *argv[])
{
    ...
    max_mem_refs=KnobMaxMemRefs.Value();
